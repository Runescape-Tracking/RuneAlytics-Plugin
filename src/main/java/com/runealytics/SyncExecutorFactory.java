...
public class SyncExecutorFactory
{
	...
	public static void shutdown(ScheduledExecutorService executor)
	{
		...
		try
		{
			...
			{
				log.warn("[sync-executor] Sync executor did not terminate in 5s, forcing shutdown");
		...
		{
			log.warn("[sync-executor] Interrupted while shutting down sync executor: {}", e.getMessage());
			executor.shutdownNow();
		}
	}
}
